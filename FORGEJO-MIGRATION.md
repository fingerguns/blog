...
| 4 | `triggerRebuild()` — deploy hook, with a GitHub `workflow_dispatch` fallback | `worker/update-thinking.js:627-655` |
| 5 | Worker vars `GITHUB_OWNER` / `GITHUB_REPO` / `GITHUB_BRANCH`, secret `GITHUB_TOKEN` | `worker/wrangler.toml:11-13,58` |
| 6 | Changelog entries link to `github.com/.../commit/{hash}` | `scripts/build.mjs:1944` |
...
`triggerRebuild()` at `worker/update-thinking.js:627-655` currently tries `PAGES_DEPLOY_HOOK`, then falls back to GitHub `workflow_dispatch`. After migration the hook is dead and the fallback becomes the only path.
