...
**The docs drift sweep stops working.** Routine `trig_01RGunVKiR5vRmb56tVDoWjA` clones `https://github.com/fingerguns/blog`, and the API refused to save it until a GitHub account was connected — the create call failed with `Connect your GitHub account before saving a routine that uses a GitHub repository` until `/web-setup` ran. Whether `sources[].git_repository.url` accepts an arbitrary Forgejo URL is **untested and I do not know the answer**. Test it cheaply, before migrating, by creating a throwaway routine pointing at a public Codeberg repo. If it is GitHub-only, the sweep's options are: keep a GitHub mirror purely to feed it, or move the sweep to a Forgejo Actions scheduled workflow — which is only viable under Option B, and only with a runner that is awake on Mondays.
