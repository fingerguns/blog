...
| 4 | `triggerRebuild()` — deploy hook, with a GitHub `workflow_dispatch` fallback | `worker/update-thinking.js:612-675` |
...
`triggerRebuild()` at `worker/update-thinking.js:612-675` currently tries `PAGES_DEPLOY_HOOK`, then falls back to GitHub `workflow_dispatch`. After migration the hook is dead and the fallback becomes the only path.
