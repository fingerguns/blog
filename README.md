...
- **Editing published posts** — the Writing tab's **Published** sub-tab lists every published post (newest first) with a link to the live page and an Edit button that loads it back into Compose. Post pages also carry an `edit` link beside the reading time, injected client-side only when a signed-in admin session is present in that browser — both routes land on `/admin/?post=<slug>`, and the Worker still checks the password on `fetch-post`/`edit-post`
