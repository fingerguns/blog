...
- **Job status** — Cron syncs, rebuilds, and syndication attempts record their outcome to D1 (`job_runs`); `/admin/` shows a health strip that names anything failed or stale, so a silent background failure stops being silent
