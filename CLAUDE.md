...
**Build-time external-data caching is a repeated pattern, not a one-off.** Several build steps look up data from external services once and cache the result in a `data/*.json` file keyed by URL/id, so subsequent builds only fetch what's new: Spotify thumbnails (`data/spotify-thumbnails.json`), video-poster existence checks (`data/video-posters.json`), Reading cover art (`data/reading-covers.json`), and Sharing-link Open Graph unfurls (`data/linklog-unfurls.json`). These caches are committed to git (they're cheap to regenerate but expensive to refetch). All of them run through `scripts/lib/concurrency.mjs`'s `mapWithConcurrency` (concurrency cap of 6) rather than fetching sequentially — follow that pattern for any new build-time external lookup.
...
**AI tagging happens at write time in the Worker, not at build time.** Must Reads genres (`worker/reading-genres.mjs`) and Sharing topic tags (`worker/linklog-tags.mjs`) both follow the same shape: after the D1 insert, `schedule*Assignment(ctx, env, db, id, triggerRebuild)` fires a `ctx.waitUntil()` background task that calls Claude Fable, saves the result to D1, and triggers a second Pages rebuild so the tag shows up without the admin waiting on the request. Each has a matching `backfill-*` Worker action (and `npm run backfill-*` wrapper) for entries that predate the feature or whose tagging failed silently (no `ANTHROPIC_API_KEY`, a bad model response, etc.) — those are retried by rerunning the backfill, not automatically. If you add a new AI-tagged field, wire it into the create handler this way rather than as a build-time step.

**Sharing (linklog) has two renderings from one dataset.** The homepage section (`renderLinklogItem`) is always plain date+link. The `/sharing/` archive (`renderLinklogArchiveItem`) renders unfurl cards (image/description/source) sourced from `data/linklog-unfurls.json`, with a tag-filter dropdown built from `linklog.tags`. Don't conflate the two renderers when touching either.
